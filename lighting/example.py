...
while True:
    ...
    light_box.update_dynamic_walls(moving_box_id,
                                   lighting.box([1200 + math.sin(timer / 100) * 50, 100 + math.sin(timer / 72) * 100],
                                                [(1 + math.sin(timer / 60)) * 50, (1 + math.sin(timer / 65)) * 50]))
    ...
    # Update Lights ------------------------------------------ #
    mouse_light_offsets = [[(i % 3 - 1) * math.sin(timer / 40) * 60, (i // 3 - 1) * math.sin(timer / 40) * 60] for i in
                           range(9)]
    ...
    for i, light in enumerate(mouse_lights):
        ...
        light_box.get_light(light).position = [offset[0] + mx + mouse_light_offsets[i][0],
                                               offset[1] + my + mouse_light_offsets[i][1]]
